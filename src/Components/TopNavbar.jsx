...
const notifications = [
  { id: 1, title: '📩 New Message Received', content: 'You have a new message from Alice.' },
  {
    ...
    content: 'A new system update is ready to install.'
  },
  ...
  return (
    <div className='w-full   h-[92px] border-l border-[#262A5B] bg-[#12161F] shadow-md p-4 md:hidden block md:justify-between justify-end items-center'>
      ...
      <div className=' items-center hidden space-x-3 text-white'>
        ...
        <Link
          to='../settings'
          className='cursor-pointer bg-[#712fff1a] p-[14px] rounded-full text-xl'
        >
          <RiSettings2Line />
        </Link>
          ...
          <Menu.Items className='absolute z-50 right-0 mt-2 w-64 bg-white text-gray-800 shadow-lg rounded-md overflow-hidden'>
            ...
            <Menu.Item>
              {({ active }) => (
                <Link
                  to='/notifications'
                  className={`block text-center text-blue-500 px-4 py-2 font-medium ${
                    active ? 'bg-gray-200' : ''
                  }`}
                >
                  View All
        ...
        <Menu as='div' className='relative'>
          <Menu.Button className='cursor-pointer flex items-center gap-2 bg-[#050611] p-[8px] rounded-full text-xl'>
            <img
              src={`${`https://api.dicebear.com/6.x/adventurer/svg?seed=male`}`}
              alt='Avatar'
              className='w-8 h-8'
            />
          ...
          <Menu.Items className='absolute z-50 right-0 mt-2 w-40 bg-white text-gray-800 shadow-lg rounded-md overflow-hidden'>
            <Menu.Item>
              {({ active }) => (
                <Link
                  to='#'
                  className={`block cursor-pointer px-4 py-2 ${active ? 'bg-gray-200' : ''}`}
                >
                  Profile
                </Link>
              )}
            ...
            <Menu.Item>
              {({ active }) => (
                <button
                  onClick={logout}
                  className={`w-full  cursor-pointer text-left px-4 py-2 text-red-500 ${
                    active ? 'bg-gray-200' : ''
                  }`}
                >
                  Logout
